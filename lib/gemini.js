...
export function getGeminiModelEndpoint(model = "gemini-2.5-pro") {
  const cleanModel = (model || "gemini-2.5-pro").trim();
 ...
 */
export async function callGemini(apiKey, prompt, systemInstruction = "", responseMimeType = "text/plain", model = "gemini-2.5-pro") {
  if (!apiKey || !apiKey.trim()) {
    throw new Error("Missing Gemini API key. Please configure your API key in TabZen Settings.");
  }

  const endpoint = getGeminiModelEndpoint(model);
...
/**
 * Cluster tabs into meaningful topic groups using Gemini Pro / Flash
 */
export async function clusterTabsWithAI(apiKey, tabs, model = "gemini-2.5-pro") {
 ...
 */
export async function getTabTakeaway(apiKey, tabInfo, model = "gemini-2.5-pro") {
  if (!apiKey || !apiKey.trim()) {
    return {
 ...
 */
export async function getDeepSummary(apiKey, tabInfo, model = "gemini-2.5-pro") {
  if (!apiKey || !apiKey.trim()) {
    return {
