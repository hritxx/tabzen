...
export function normalizeModelName(model) {
  ...
  // Auto-migrate deprecated 2.5 / 1.5 models to active 3.x models
  if (m.includes("2.5-flash") || m.includes("1.5-flash") || m.includes("2.0-flash")) {
    return "gemini-3.6-flash";
  }
  if (m === "gemini-3-pro" || m === "gemini-3-flash") {
    // If user picked generic 3-pro/3-flash, map to known 3.x models
 ...
 */
export async function callGemini(apiKey, prompt, systemInstruction = "", responseMimeType = "text/plain", model = "gemini-3.6-flash") {
  ...
  // If model is not found (404/400) and was not gemini-3.6-flash, automatically fallback!
  if (!response.ok && (response.status === 404 || response.status === 400) && cleanModel !== "gemini-3.6-flash") {
    console.warn(`Model ${cleanModel} returned ${response.status}. Automatically retrying with gemini-3.6-flash...`);
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.groups) && parsed.groups.length > 0) {
      ...
      if (missingTabs.length > 0) {
        // Classify any missed tabs by heuristic and merge
...
export function clusterTabsHeuristic(tabs) {
  ...
  // For any remaining tabs, group by domain hostname
  const domainBuckets = {};
  for (const tabId of remaining) {
    const tab = tabMap.get(tabId);
    let domain = "Browsing";
 ...
 */
export async function getTabTakeaway(apiKey, tabInfo, model = "gemini-3.6-flash") {
  if (!apiKey || !apiKey.trim()) {
    return {
 ...
 */
export async function getDeepSummary(apiKey, tabInfo, model = "gemini-3.6-flash") {
  if (!apiKey || !apiKey.trim()) {
    return {
