...
function formatTab(tab, tabAccessTimes) {
  // Unknown age (never recorded, no native lastAccessed) counts as 0, not
  // now: an untracked tab is neglected by definition, so it sorts first and
  // qualifies for the stale filter instead of silently disappearing from it.
  const lastAccessed = tabAccessTimes[tab.id] ?? tab.lastAccessed ?? 0;
